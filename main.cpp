/* (C) Louis Höfler
 * Original github repository https://github.com/louiswork/sneditor
 * This Programm is FREE. The author(s) are not liable for any error in the software
 * and the usage of the software. The author does not guarantee dataloss.
 * 
 * LICENSETERMS
 * Modification of this software is ONLY permitted if the whole modified code
 * is published and a Link is added to the original github repository.
 * Use the github bug report feature in the above original repository to publish the link. 
 * Private and Commercial usage is Permitted. In booth cases modifications to the software need to
 * be made public to the above github repository.
 * This comment block has to be preserved and is not allowed to be deleted.
 */
...
/* treeItemData
 * This class holds the data of the tree items.
 * There is a path to the file and the name.
 */
class treeItemData : public wxTreeItemData {
 public:
  wxString m_path;
  wxString m_name;
  treeItemData(wxString path, wxString name) : m_path(path), m_name(name) {}
  wxString GetPath() {
   return m_path;
  }
  wxString GetName() {
...
/* buttonData
 * This class holds the properties if
 * a button is clicked on a XML parsed button pane
 */
class ButtonData : public wxObject {
 public:
  ...
  wxString m_type;
  wxString m_content;
  ButtonData(wxString label, wxString type, wxString content) : m_label(label), m_type(type), m_content(content) {};
};

/* This class holds the data for the parsed xml.
 * For every xml element there is a button created,
 * on click of the button an editor pane is created
 * that contains the editor for javascript etc.
 */
class XmlTreeItem {
 public:
  wxButton* m_button;
  ...
  XmlTreeItem(wxString label, wxString type, wxString content, unsigned int align) : m_align(align) {
   m_data = new ButtonData(label, type, content);
  }
};
...
private:
 ...
 void TreeItemDoubleClick(wxTreeEvent& event);
 wxString PrettyPrint(const wxString& in);
 tree<XmlTreeItem*> ParseXML(wxString path);
public:
 void XmlButtonClicked(wxCommandEvent& event);
private:
 wxAuiManager m_mgr;
...
void MyFrame::XmlButtonClicked(wxCommandEvent& event) {
...
class wxSizeReportCtrl : public wxControl {
 public:
  wxSizeReportCtrl(wxWindow* parent, wxWindowID id = wxID_ANY, const wxPoint& pos = wxDefaultPosition, const wxSize& size = wxDefaultSize, wxAuiManager* mgr = NULL) : wxControl(parent, id, pos, size, wxNO_BORDER) {
   m_mgr = mgr;
  }
 private:
...
class SettingsPanel : public wxPanel {
 ...
 public:
  SettingsPanel(wxWindow* parent, MyFrame* frame) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize), m_frame(frame) {
...
bool MyApp::OnInit() {
 if(!wxApp::OnInit())
  return false;
 wxFrame* frame = new MyFrame(NULL, wxID_ANY, "wxAUI Sample Application", wxDefaultPosition, wxWindow::FromDIP(wxSize(800, 600), NULL));
 frame->Show();
 return true;
}

wxString MyFrame::PrettyPrint(const wxString& in) {
...
bool IsText(wxString cnt) {
 return false;
}

bool IsSysId(wxString cnt) {
 return false;
}

bool IsJavaScript(wxString cnt) {
 return true;
}

tree<XmlTreeItem*> MyFrame::ParseXML(wxString path) {
 tree<XmlTreeItem*> ret;
 tree<XmlTreeItem*>::iterator itrroot;
 tree<XmlTreeItem*>::iterator itr;
 unsigned int align = 0;
 //XML parse
 wxXmlDocument doc;
 if(!doc.Load(path))
  return ret;
 if(doc.GetRoot()->GetName() != "unload")
  return ret;
 XmlTreeItem* ritm = new XmlTreeItem(doc.GetRoot()->GetName(), "string", doc.GetRoot()->GetContent(), align);
 ritm->m_button->Connect(wxEVT_BUTTON, wxCommandEventHandler(MyFrame::XmlButtonClicked), NULL, this);
 itrroot = ret.insert(ret.begin(), ritm);
 wxXmlNode *child = doc.GetRoot()->GetChildren();
 while(child) {
  align++;
  if(child->GetName() == "sys_remote_update_set") {
  } else if(child->GetName() == "application") {
  } else if(child->GetName() == "application_name") {
  } else if(child->GetName() == "application_scope") {
  } else if(child->GetName() == "description") {
    XmlTreeItem* itm = new XmlTreeItem(doc.GetRoot()->GetName(), "text", doc.GetRoot()->GetContent(), align);
    itm->m_button->Connect(wxEVT_BUTTON, wxCommandEventHandler(MyFrame::XmlButtonClicked), NULL, this);
    itr = ret.insert(ret.begin(), itm);
  } else if(child->GetName() == "name") {
    XmlTreeItem* itm = new XmlTreeItem(doc.GetRoot()->GetName(), "text", doc.GetRoot()->GetContent(), align);
    itm->m_button->Connect(wxEVT_BUTTON, wxCommandEventHandler(MyFrame::XmlButtonClicked), NULL, this);
    itr = ret.insert(ret.begin(), itm);
  } else if(child->GetName() == "remote_sys_id") {
    XmlTreeItem* itm = new XmlTreeItem(doc.GetRoot()->GetName(), "sysid", doc.GetRoot()->GetContent(), align);
    itm->m_button->Connect(wxEVT_BUTTON, wxCommandEventHandler(MyFrame::XmlButtonClicked), NULL, this);
    itr = ret.insert(ret.begin(), itm);
  } else if(child->GetName() == "payload") {
   if(IsText(child->GetContent())) {
    XmlTreeItem* itm = new XmlTreeItem(doc.GetRoot()->GetName(), "text", doc.GetRoot()->GetContent(), align);
    itm->m_button->Connect(wxEVT_BUTTON, wxCommandEventHandler(MyFrame::XmlButtonClicked), NULL, this);
    itr = ret.insert(ret.begin(), itm);
   } else if(IsSysId(child->GetContent())) {
    XmlTreeItem* itm = new XmlTreeItem(doc.GetRoot()->GetName(), "sysid", doc.GetRoot()->GetContent(), align);
    itm->m_button->Connect(wxEVT_BUTTON, wxCommandEventHandler(MyFrame::XmlButtonClicked), NULL, this);
    itr = ret.insert(ret.begin(), itm);
   } else if(IsJavaScript(child->GetContent())) {
    XmlTreeItem* itm = new XmlTreeItem(doc.GetRoot()->GetName(), "javascript", doc.GetRoot()->GetContent(), align);
    itm->m_button->Connect(wxEVT_BUTTON, wxCommandEventHandler(MyFrame::XmlButtonClicked), NULL, this);
    itr = ret.insert(ret.begin(), itm);
   }
  }
...
void MyFrame::TreeItemDoubleClick(wxTreeEvent& event) {
 wxBitmap page_bmp = wxArtProvider::GetBitmap(wxART_NORMAL_FILE, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxTreeItemId itemId = event.GetItem();
 treeItemData* data = (treeItemData*)m_tree->GetItemData(itemId);
 //Read file content
 wxString content;
 wxFileInputStream input(data->GetPath());
 wxTextInputStream text(input, wxT("\x09"), wxConvUTF8);
 while(input.IsOk() && !input.Eof()) {
  content += text.ReadLine();
 }
 wxString pretty = PrettyPrint(content);
 tree<XmlTreeItem*> xmlTree = ParseXML(data->GetPath());
 m_notebook->AddPage(new wxTextCtrl(m_notebook, wxID_ANY, pretty, wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxNO_BORDER), data->GetName(), false, page_bmp);
}

wxBEGIN_EVENT_TABLE(MyFrame, wxFrame)
 EVT_TREE_ITEM_ACTIVATED(wxID_ANY, MyFrame::TreeItemDoubleClick)
...
MyFrame::MyFrame(wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos, const wxSize& size, long style) : wxFrame(parent, id, title, pos, size, style) {
 ...
 wxBitmap tb2_open = wxArtProvider::GetBitmap(wxART_FOLDER_OPEN, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_close = wxArtProvider::GetBitmap(wxART_CLOSE, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_save = wxArtProvider::GetBitmap(wxART_FILE_SAVE, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_saveas = wxArtProvider::GetBitmap(wxART_FILE_SAVE_AS, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_undo = wxArtProvider::GetBitmap(wxART_UNDO, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_redo = wxArtProvider::GetBitmap(wxART_REDO, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_cut = wxArtProvider::GetBitmap(wxART_CUT, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_copy = wxArtProvider::GetBitmap(wxART_COPY, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_paste = wxArtProvider::GetBitmap(wxART_PASTE, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_reload = wxArtProvider::GetBitmap(wxART_PASTE, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_find = wxArtProvider::GetBitmap(wxART_FIND, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_replace = wxArtProvider::GetBitmap(wxART_FIND_AND_REPLACE, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_settings = wxArtProvider::GetBitmap(wxART_HELP_SETTINGS, wxART_OTHER, FromDIP(wxSize(16,16)));
 wxBitmap tb2_help = wxArtProvider::GetBitmap(wxART_QUESTION, wxART_OTHER, FromDIP(wxSize(16,16)));
 tb2->AddTool(ID_SampleItem+6, "Open", tb2_open);
 tb2->AddTool(ID_SampleItem+7, "Close", tb2_close);
 tb2->AddTool(ID_SampleItem+8, "Save", tb2_save);
 tb2->AddTool(ID_SampleItem+9, "SaveAs...", tb2_saveas);
 tb2->AddSeparator();
 tb2->AddTool(ID_SampleItem+10, "Undo", tb2_undo);
 tb2->AddTool(ID_SampleItem+11, "Redo", tb2_redo);
 tb2->AddTool(ID_SampleItem+12, "Copy", tb2_copy);
 tb2->AddTool(ID_SampleItem+13, "Cut", tb2_cut);
 tb2->AddTool(ID_SampleItem+13, "Paste", tb2_paste);
 tb2->AddSeparator();
 tb2->AddTool(ID_SampleItem+14, "Find", tb2_find);
 tb2->AddTool(ID_SampleItem+15, "Replace", tb2_replace);
 tb2->AddSeparator();
 tb2->AddTool(ID_SampleItem+16, "Settings", tb2_settings);
 tb2->AddTool(ID_SampleItem+17, "Help", tb2_help);
 ...
 //tb4->Realize();
 //wxAuiToolBar* tb5 = new wxAuiToolBar(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxAUI_TB_DEFAULT_STYLE | wxAUI_TB_OVERFLOW | wxAUI_TB_VERTICAL);
 ...
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("test1").Caption("Pane Caption").Top());
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("test2").Caption("Client Size Reporter").Bottom().Position(1).CloseButton(true).MaximizeButton(true));
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("test3").Caption("Client Size Reporter").Bottom().CloseButton(true).MaximizeButton(true));
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("test4").Caption("Pane Caption").Left());
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("test5").Caption("No Close Button").Right().CloseButton(false));
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("test6").Caption("Client Size Reporter").Right().Row(1).CloseButton(true).MaximizeButton(true));
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("test7").Caption("Client Size Reporter").Left().Layer(1).CloseButton(true).MaximizeButton(true));
 ...
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("test9").Caption("Min Size 200x100").BestSize(FromDIP(wxSize(200,100))).MinSize(FromDIP(wxSize(200,100))).Bottom().Layer(1).CloseButton(true).MaximizeButton(true));
 ...
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("test11").Caption("Fixed Pane").Bottom().Layer(1).Position(2).Fixed());
 ...
 m_mgr.AddPane(CreateGrid(), wxAuiPaneInfo().Name("grid_content").CenterPane().Hide());
 m_mgr.AddPane(CreateTreeCtrl(), wxAuiPaneInfo().Name("tree_content").CenterPane().Hide());
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Name("sizereport_content").CenterPane().Hide());
...
void MyFrame::OnCreatePerspective(wxCommandEvent& WXUNUSED(event)) {
 wxTextEntryDialog dlg(this, "Enter a name for the new perspective:", "wxAUI Test");
...
void MyFrame::OnNotebookPageClose(wxAuiNotebookEvent& evt) {
 ...
 if(ctrl->GetPage(evt.GetSelection())->IsKindOf(CLASSINFO(wxHtmlWindow))) {
  int res = wxMessageBox("Are you sure you want to close/hide this notebook page?", "wxAUI", wxYES_NO, this);
  if(res != wxYES)
   evt.Veto();
 }
}
...
void MyFrame::OnCreateTree(wxCommandEvent& WXUNUSED(event)) {
 m_mgr.AddPane(CreateTreeCtrl(), wxAuiPaneInfo().Caption("Tree Control").Float().FloatingPosition(GetStartPosition()).FloatingSize(FromDIP(wxSize(150,300))));
 m_mgr.Update();
}

void MyFrame::OnCreateGrid(wxCommandEvent& WXUNUSED(event)) {
 m_mgr.AddPane(CreateGrid(), wxAuiPaneInfo().Caption("Grid").Float().FloatingPosition(GetStartPosition()).FloatingSize(FromDIP(wxSize(300,200))));
 m_mgr.Update();
}

void MyFrame::OnCreateHTML(wxCommandEvent& WXUNUSED(event)) {
 m_mgr.AddPane(CreateHTMLCtrl(), wxAuiPaneInfo().Caption("HTML Control").Float().FloatingPosition(GetStartPosition()).FloatingSize(FromDIP(wxSize(300,200))));
 m_mgr.Update();
}

void MyFrame::OnCreateNotebook(wxCommandEvent& WXUNUSED(event)) {
 //m_mgr.AddPane(CreateNotebook(), wxAuiPaneInfo().Caption("Notebook").Float().FloatingPosition(GetStartPosition()).FloatingSize(FromDIP(wxSize(300,200))).CloseButton(true).MaximizeButton(true));
 m_mgr.AddPane(CreateNotebook(), wxAuiPaneInfo().Caption("Notebook").Float().FloatingPosition(GetStartPosition()).CloseButton(true).MaximizeButton(true));
 m_mgr.Update();
}

void MyFrame::OnCreateText(wxCommandEvent& WXUNUSED(event)) {
 m_mgr.AddPane(CreateTextCtrl(), wxAuiPaneInfo().Caption("Text Control").Float().FloatingPosition(GetStartPosition()));
 m_mgr.Update();
}

void MyFrame::OnCreateSizeReport(wxCommandEvent& WXUNUSED(event)) {
 m_mgr.AddPane(CreateSizeReportCtrl(), wxAuiPaneInfo().Caption("Client Size Reporter").Float().FloatingPosition(GetStartPosition()).CloseButton(true).MaximizeButton(true));
 m_mgr.Update();
}
...
class wxDirTraverserSimple : public wxDirTraverser {
 ...
 public:
  ...
  wxTreeItemId m_id;
  wxSize m_size;
  wxDirTraverserSimple(wxArrayString& files, wxTreeCtrl* tree, wxSize size) : m_files(files), m_tree(tree), m_size(size) {
   ...
   m_root = m_tree->AddRoot("root", 0);
   m_id = m_root;
  }
  virtual wxDirTraverseResult OnFile(const wxString& filepath) {
   wxFileName path(filepath);
   wxTreeItemData* data = new treeItemData(filepath, path.GetName());
   m_tree->AppendItem(m_id, path.GetName(), 1, -1, data);
   return wxDIR_CONTINUE;
  }
  virtual wxDirTraverseResult OnDir(const wxString& dirpath) {
   wxFileName path(dirpath);
   wxTreeItemData* data = new treeItemData(dirpath, path.GetName());
   m_items.Add(m_tree->AppendItem(m_id, path.GetName(), 0, -1, data));
   m_id = m_items.Item(m_items.Count());
   return wxDIR_CONTINUE;
  }
  wxTreeItemId returnRoot() {
   return m_root;
  }
};

wxTreeCtrl* MyFrame::CreateTreeCtrl() {
 m_tree = new wxTreeCtrl(this, wxID_ANY, wxPoint(0,0), FromDIP(wxSize(160,250)), wxTR_DEFAULT_STYLE | wxNO_BORDER);
 wxSize size = FromDIP(wxSize(16, 16));
 //Prepare wxdirtraverse
 wxArrayString files;
 wxDirTraverserSimple traverser(files, m_tree, size);
 wxDir dir("C:\\lulu\\bin\\sneditor\\updatesets");
 dir.Traverse(traverser);
 wxTreeItemId root = traverser.returnRoot();
 m_tree->Expand(root);
 return m_tree;
}

wxSizeReportCtrl* MyFrame::CreateSizeReportCtrl(const wxSize& size) {
 wxSizeReportCtrl* ctrl = new wxSizeReportCtrl(this, wxID_ANY, wxDefaultPosition, size, &m_mgr);
 return ctrl;
}
...
wxAuiNotebook* MyFrame::CreateNotebook() {
 ...
 m_notebook = new wxAuiNotebook(this, wxID_ANY, wxPoint(client_size.x, client_size.y), FromDIP(wxSize(430,200)), m_notebook_style);
 m_notebook->Freeze();
 ...
 //flex->Add(new wxTextCtrl( panel, -1, "", wxDefaultPosition, FromDIP(wxSize(100,-1))), 1, wxALL|wxALIGN_CENTRE, FromDIP(5));
 //flex->Add(new wxStaticText( panel, -1, "wxSpinCtrl:" ), 0, wxALL|wxALIGN_CENTRE, FromDIP(5));
 //flex->Add(new wxSpinCtrl( panel, -1, "5", wxDefaultPosition, wxDefaultSize, wxSP_ARROW_KEYS, 5, 50, 5 ), 0, wxALL|wxALIGN_CENTRE, FromDIP(5));
 //flex->Add(FromDIP(5), FromDIP(5) );   flex->Add( FromDIP(5), FromDIP(5) );
 //panel->SetSizer(flex);
 //ctrl->AddPage(panel, "wxPanel", false, page_bmp);
 //m_notebook->AddPage(new wxTextCtrl(m_notebook, wxID_ANY, "Some text", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE | wxNO_BORDER), "wxTextCtrl 1", false, page_bmp);
 
 //ctrl->AddPage( new wxTextCtrl(ctrl, wxID_ANY, "Some more text", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxNO_BORDER) , "wxTextCtrl 2");
 //ctrl->AddPage( new wxTextCtrl(ctrl, wxID_ANY, "Some more text", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxNO_BORDER) , "wxTextCtrl 3");
 //ctrl->AddPage( new wxTextCtrl(ctrl, wxID_ANY, "Some more text", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxNO_BORDER) , "wxTextCtrl 4");
 //ctrl->AddPage( new wxTextCtrl(ctrl, wxID_ANY, "Some more text", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxNO_BORDER) , "wxTextCtrl 5");
 //ctrl->AddPage( new wxTextCtrl(ctrl, wxID_ANY, "Some more text", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxNO_BORDER) , "wxTextCtrl 6");
 //ctrl->AddPage( new wxTextCtrl(ctrl, wxID_ANY, "Some more text", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxNO_BORDER) , "wxTextCtrl 7 (longer title)" );
 //ctrl->SetPageToolTip(ctrl->GetPageCount()-1, "wxTextCtrl 7: and the tooltip message can be even longer!");
 //ctrl->AddPage( new wxTextCtrl(ctrl, wxID_ANY, "Some more text", wxDefaultPosition, wxDefaultSize, wxTE_MULTILINE|wxNO_BORDER) , "wxTextCtrl 8" );
 m_notebook->Thaw();
 return m_notebook;
}
